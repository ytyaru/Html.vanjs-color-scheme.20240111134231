...
//        console.log('夏至〜冬至の日入差:', this.#span(sumSet, winSet)/1000/60)
        ...
        const diff = {'sunrise':this.#getTimeSpan(winRise, sumRise), 'sunset':this.#getTimeSpan(sumSet, winSet)}
...
//        const sunset = new Date(sumSet.getTime() - (diffNowDays * daylyValue.sunset))
    ...
    #nearSummer() { // 夏至／冬至のどちらに近いか。近いほうのsunrise/sunsetを使う。加算・減算が逆転する。
        ...
        const winterSolF = new Date(`${this._launched.getFullYear()+1}-${SunshineData.Solstice.winter}`)
        const now = new Date()
        const diffSumNow = this.#span(summerSol, now)
        const diffWinNow = this.#span(winterSol, now)
        const diffWinFNow = this.#span(winterSolF, now)
        console.log('diffSum Now:', diffSumNow, summerSol)
        console.log('diffWin Now:', diffWinNow, winterSol)
        console.log('diffWinFNow:', diffWinFNow, winterSolF)
        console.log('now       :', now.getTime())
        console.log((diffSumNow < diffWinNow))
        if (diffSumNow < diffWinNow && diffSumNow < diffWinFNow) { return [true,  diffSumNow] }
...
//        const _d2 = new Date(`2000-01-01 ${d2.getHours().toString().toString().padStart(2,'0')}:${d2.getMinutes().toString().padStart(2,'0')}:${d2.getSeconds().toString().padStart(2,'0')}`)
        const _d1 = new Date(`2000-01-01 ${this.#hhmmss(d1)}`)
        const _d2 = new Date(`2000-01-01 ${this.#hhmmss(d2)}`)
        console.log(`2000-01-01 ${this.#hhmmss(_d1)}`)
        console.log(_d1)
        console.log(_d2)
